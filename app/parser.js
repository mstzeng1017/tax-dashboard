...
(function () {
  ...
  // === Parser: 各類所得資料清單 (optional) ===
  // We only care about the total withheld tax across all rows.
  function parseIncomeList(text, lines) {
    ...
    out.taxpayer = findName(text, ['納稅義務人姓名', '納稅義務人', '所得人']);

    // Try labeled total first
    out.totalWithheld = findLabeled(text, [
      '全戶.{0,4}扣繳稅額.{0,8}合計',
      '扣繳稅額.{0,8}合計',
      '已扣繳稅額.{0,8}合計',
      '全戶扣繳稅額',
    ...
    // Fallback: sum the last column of every detail row
    if (out.totalWithheld == null) {
      let total = 0;
      let any = false;
      for (const line of lines) {
        // Heuristic detail row: contains a tax category code at start
        if (!/^(?:\s)?(50|5A|5B|71|71G|76|9[ABCD]|9[1-6])\b/.test(line)) continue;
        const numbers = (line.match(/[\d,]{1,}/g) || []).map(num).filter((v) => v !== null && v >= 0);
        if (numbers.length < 2) continue;
        // Last number = withheld in standard layout
        total += numbers[numbers.length - 1];
        any = true;
      }
      if (any) out.totalWithheld = total;
    }
